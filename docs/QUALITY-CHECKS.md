...
clause-local ordered recommendation/obligation or permission/obligation drift, loss of recommendation polarity, `muss nicht`/no-obligation strengthening, loss of an explicit prohibition, or loss
of source-critical currency/percentage/identifier values is withheld for review rather than silently
returned. Multiple protected modalities inside one clause are compared in event order rather than as an unordered category set. Currency/percentage checks compare conservative numeric interpretation sets, so locale-equivalent unambiguous formatting and currency-code placement remain accepted while ambiguous single-separator three-decimal forms (`1,234` / `1.234`) cannot silently collapse to a 1000x-different value. The runtime filter does

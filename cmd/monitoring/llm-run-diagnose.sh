...
exp "Mesa 26.1.4 | governor 0.4.12 | package-standard Ollama 0.34.0. Other Ollama versions are explicit comparison runs."
...
if [[ "$ov" == "0.34.0" ]]; then ok "ollama=$ov (package standard)"
elif [[ -n "$ov" ]]; then wn "ollama=$ov (package standard 0.34.0; compare results as a runtime override)"

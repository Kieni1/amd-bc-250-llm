#!/usr/bin/env bash
# llm-run-diagnose.sh - BC-250 performance-parity CHECKER.
#
...
#   MODEL=prod-gemma4-e4b-unsloth-qat-ud-q4-k-xl sudo llm-run-diagnose
#
# ===========================================================================
...
echo "############################################################"
echo "# BC-250 parity check - $(hostname) - $(date --iso-8601=seconds)"
...
exp "Mesa 26.1.4 | governor 0.4.12 | package-standard Ollama 0.33.3. Other Ollama versions are explicit comparison runs."
...
if [[ "$ov" == "0.33.3" ]]; then ok "ollama=$ov (package standard)"
elif [[ -n "$ov" ]]; then wn "ollama=$ov (package standard 0.33.3; compare results as a runtime override)"
...
sec "9. SUSTAINED LOAD - skipped because --no-load was requested"
fi

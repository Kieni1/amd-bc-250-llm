# BC-250 0.12.2-0.2 release testing handover

## Scope

Build and test the exact `bc250-llm-server-0.12.2-0.2` candidate. This is a new functional/runtime
...
0.12.2-0.2 also closes the pre-device logic review: complete French modality conjugations,
leading-zero decimal integrity, one production translation prompt/wrapper authority across direct and
OWUI qualification, direct `modality` taxonomy, fail-closed `ordinary_user_visible` typing, and
ACL/xattr-preserving migration rollback archives with a documented restore path. Include focused
checks of those repaired boundaries in the source/build evidence.
...
```text
install new RPM
-> existing OWUI boot enablement is held
-> old running service may remain until guided convergence, but reboot must not auto-start the new image
-> bc250-install stops OWUI before migration
...
unstarted and package boot enablement held.
...
Correct locale-equivalent CHF/date formatting is not a semantic failure. If the underlying model

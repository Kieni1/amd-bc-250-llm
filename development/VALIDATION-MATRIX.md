...
Current source baseline while this file was refreshed: `0.11.3-2.4` source release.
Release 2.4 carries the device-accepted 2.3 operations behavior forward and contains two bounded
follow-ups: the pinned-upstream reboot-path correction and a small extension of the existing Open
WebUI desired-state contract. Production model choices, normal/agent topology, hard resource
thresholds, power policy, RAG/MTP decisions and GGUF provenance are unchanged.
...
|---|---|---|---|---|
| repository/unit validation | SOURCE PASS — RPM preflight PASS, deterministic suite 447/447 PASS in split modules, bash -n 64/64, Python compileall PASS | PENDING/EXTERNAL | EXACT 2.3 targeted operations acceptance PASS | source release frozen; RPM/SRPM build remains external |
| RPM build/install | source metadata targets 0.11.3-2.4; RPM/SRPM build remains an external package-build gate | PENDING/EXTERNAL | EXACT 2.3 — installed targeted operations acceptance PASS; exact 1.7 remains newest full whole-appliance revalidation | build/install exact 2.4, then run bounded changed-surface acceptance |
...
| Open WebUI product surface / desired state | SOURCE PASS — 2.4 owns Arena-off, hidden-active implementation models, persisted local/offline policy and upload limits/extensions through the existing authenticated apply/status path; pinned-v0.11.3 OpenAI-style adapter limitations are explicitly documented, not vendor-patched | external | EXACT 2.3 — HTTP/task/curated roles/translation/RAG/direct switching/concurrency/topology restoration healthy; product-state gaps identified | build/install 2.4, verify clean status/UI, active-but-hidden implementation models and harmless drift reconvergence; no broad model campaign |

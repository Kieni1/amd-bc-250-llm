...
Current source baseline while this file was refreshed: `0.12.2-0.1`. Historical rows below retain the
...
Advanced `think=true` candidate, two 8K pressure-profile resets and migration-safe OWUI upgrade handling),
so those surfaces are PENDING real-device qualification.
...
|---|---|---|---|---|
| repository/unit validation | SOURCE PASS — static RPM preflight, shell/Python/JSON syntax checks and focused affected-module tests pass after the final model-retirement/visibility cleanup; current deterministic inventory is 482 methods and the full suite was intentionally not rerun in this integration pass; `development/` remains excluded from production discovery | PENDING/EXTERNAL | 0.12.1-0.6 exact device baseline available | build exact 0.12.2-0.1, then run the crossed-boundary release handover |
| RPM build/install | source metadata targets 0.12.2-0.1; RPM/SRPM build remains an external package-build gate | PENDING/EXTERNAL | exact 0.12.1-0.6 is the immediate device comparison baseline | build/install exact 0.12.2-0.1, then run the crossed-boundary release handover |
...
| translation production Translate-Gemma | SOURCE PASS — package prompt/filter contract now encodes explicit `sollte -> devrait`, `devrait -> sollte`, obligation/prohibition preservation and benchmark `modality` taxonomy | external | EXACT 0.12.1-0.6 — runtime path passed but recommendation strengthening reproduced | requalify both directions on exact 0.12.2; filter withholding does not erase underlying model-quality evidence |

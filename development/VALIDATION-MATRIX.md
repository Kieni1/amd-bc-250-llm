...
|---|---|---|---|---|
| repository/unit validation | 2.3 closure: repository/RPM preflight PASS; complete deterministic suite **438/438 PASS** when run in split modules after the monolithic runner exceeded the execution window without reporting a failure; `bash -n` **64/64 PASS**; Python compileall PASS. Ruff/ShellCheck unavailable and not claimed. | PENDING/EXTERNAL | N/A | RPM/SRPM and exact-device qualification remain separate; do not turn the runner-duration issue into a product failure or weaken coverage |

...
| translation production Translate-Gemma | SOURCE PASS — package prompt/filter contract encodes explicit `sollte -> devrait`, `devrait -> sollte`, ordered clause-local modality events, recommendation polarity, `muss nicht`/no-obligation, obligation/prohibition preservation and benchmark `modality` taxonomy | external | EXACT 0.12.1-0.6 — runtime path passed but recommendation strengthening reproduced | requalify both directions on exact 0.12.2; filter withholding does not erase underlying model-quality evidence |

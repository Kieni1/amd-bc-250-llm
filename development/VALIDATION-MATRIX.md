...
hardens the still-experimental MTP catalog/runner/comparison lane and, after the first
installed 0.4 smoke, refines install-time model reconciliation: catalog suppression is
actually honored, fully current required models are summarized, compact current/deferred
rows are shorter, registration probes are bounded/agent-aware, and combined apply/refresh
can never acquire MTP. GGUF provenance/SHA policy, normal service topology, production
identities, CU/governor policy and benchmark acceptance thresholds are unchanged.

Newest install/verification evidence is a **pre-refinement same-NVR**
`bc250-llm-server-0.11.3-0.4.fc44.x86_64` device smoke. The guided install completed with
normal topology and `bc250-verify` reported 54 ok / 0 warn / 0 fail. That exact installed
build predates the final same-Release installer refinements and therefore does not qualify
the final source bytes; rebuild/reinstall is required. Newest full appliance revalidation
remains historical `bc250-llm-server-0.11.3-0.2.fc44.x86_64`: v4.2 completed with
infrastructure/restoration PASS and full coverage, with task 5/6 on `tags-de`; exact evidence
is recorded in `development/model-runs/2026-09-18-installed-0.11.3-0.2-revalidation.md`.
Older `0.11.2-*` runs remain historical evidence for their exact installed releases.

| Area | Source/static | GitHub RPM | Real BC-250 | Current interpretation / next gate |
|---|---|---|---|---|
| repository/unit validation | SOURCE PASS — exact source tree and clean release-ZIP extraction both passed `make validate`: repository/RPM preflight, packaged shell syntax and 378/378 deterministic tests | PENDING/EXTERNAL | N/A | Ruff/ShellCheck remain workstation-owned and were not run here; GitHub build remains external |
| RPM build/install | source metadata and release closure are current at 0.11.3-0.4 | PENDING/EXTERNAL for the final refined 0.11.3-0.4 bytes | HISTORICAL/PRE-REFINEMENT SAME-NVR — 0.11.3-0.4.fc44 guided install completed and verifier was 54/0/0 | rebuild/reinstall final 0.11.3-0.4 and record artifact SHA because the prior same-NVR build predates these source refinements |
...
| model lifecycle/reconciliation | SOURCE PASS — lifecycle grammar plus catalog suppression, bounded registration probing, concise current/deferred status and generic-MTP exclusion are covered by the current 378/378 source gate | external | HISTORICAL/PRE-REFINEMENT SAME-NVR — installed 0.11.3-0.4 showed packaged sources/current registrations correctly, but picker latency/noise exposed the refinements now in source | rebuild/reinstall final 0.4 and confirm the picker appears promptly, ordinary current rows are concise, agent rows are deferred and MTP is absent |
...
| MTP lifecycle/runner/catalog | SOURCE PASS — disabled four-entry catalog, exact-ID runner path, source-state/SHA verification, port/stale-process/memory/runtime-access preflight, ollama-user privilege drop and unconditional exclusion from combined apply/refresh are covered by source/static checks | external llama.cpp | PENDING runtime qualification | rebuild/reinstall final 0.11.3-0.4, then fetch/test one candidate at a time through the explicit MTP route |
